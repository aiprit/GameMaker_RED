...
public class ObjectInstanceController {
	...
	public ObjectInstanceController(Image sprite, DataObject object, DoubleProperty x, DoubleProperty y) {
		...
		view = new DraggableImage(sprite, x, y, model.getAngle(), model.isVisible(), model.getScaleX(),
				model.getScaleY(), model.getAlpha());
		myX = x;
		myY = y;
		addListeners();
	}
	...
	public ObjectInstanceController(DataInstance dataInstance) {
		...
		try {
			view = new DraggableImage(dataInstance.getImage(), myX, myY, dataInstance.getAngle(),
					dataInstance.isVisible(), dataInstance.getScaleX(), dataInstance.getScaleY(),
					dataInstance.getAlpha());
		} catch (NullPointerException e) {
			ResourceBundle resources = ResourceBundle.getBundle(ROOM_RESOURCE_FILE);
			Image sprite = new Image(
					getClass().getClassLoader().getResourceAsStream(resources.getString(DEFAULT_SPRITE)));
			view = new DraggableImage(sprite, myX, myY, dataInstance.getAngle(), dataInstance.isVisible(),
					dataInstance.getScaleX(), dataInstance.getScaleY(), dataInstance.getAlpha());
		}
		model = dataInstance;
		addListeners();
		populateInstanceSpecificParameters(dataInstance);
