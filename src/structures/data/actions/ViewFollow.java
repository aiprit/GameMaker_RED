...
public class ViewFollow extends DataAction {
	...
	public String compileSyntax() {
		...
		if (xEnable && yEnable) {
			return String.format("library.set_scroller(current(), %f, %f);", get("X %").getValue(), get("Y %").getValue());
		} else if (xEnable) {
			return String.format("library.set_scroller_x(current(), %f);", get("X %").getValue());
		} else if (yEnable) {
			return String.format("library.set_scroller_y(current(), %f);", get("Y %").getValue());
		} else {
			return "";
		}
	}
